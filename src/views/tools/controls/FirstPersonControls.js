...
export function firstPersonControls(bimengine) {
	...
	_firstPersonControls.Active = function() {
		...
		if (!_firstPersonControls.controls) {
			_firstPersonControls.controls = new FirstPersonCameraControl(bimengine.scene.camera, bimengine.scene
				.renderer.domElement, bimengine.GetAllVisibilityModel());
	...
	function CreatorRoamPcDialog() {
		...
		input_item_0.id = "checkbox_minmap"
	...
	function render() {
		...
		if (bimengine.movefirst == true) {
			renderCommand();
		} else {

		} 
		bimengine.RenderSAO.render();
	}

	function renderCommand() {
		bimengine.scene.renderer.render(bimengine.scene, bimengine.scene.camera); //执行渲染操作 
		_firstPersonControls.controls && _firstPersonControls.controls.enabled && _firstPersonControls.controls
	.update();
		// 小地图
