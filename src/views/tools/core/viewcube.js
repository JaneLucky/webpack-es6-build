...
import { InitScene, InitAxesHelper, InitLight, InitCamera, InitRenender, InitOthers } from "../initialize/InitThreejsSence.js";
...
export function ViewCube(_Engine, scene, domid) {
  ...
  _ViewCube.init = function () {
    ...
    _ViewCube.cameraOrtho = new THREE.OrthographicCamera(
  ...
  _ViewCube.cameraGoToSpecialView = function (dir) {
    ...
    _ViewCube.animateCamera(_ViewCube.scene.camera.position, tergetCamera, _ViewCube.scene.controls.target.clone(), target);
  };
  //获取相机姿态
  ...
  _ViewCube.ReductionCameraPose = function (cameraData, time = 1000, callback) {
    ...
    var quaternion = new THREE.Quaternion(
      cameraData.quaternion._x,
      cameraData.quaternion._y,
      cameraData.quaternion._z,
      cameraData.quaternion._w
    );
    ...
    _ViewCube.animateCamera(
      _ViewCube.scene.camera.position,
      position,
      _ViewCube.scene.controls.target.clone(),
      target,
      _ViewCube.scene.camera.quaternion,
      quaternion,
      time,
      res => {
  ...
  _ViewCube.cameraGoHome = function () {
    var box = _ViewCube.getBoundingBox(_Engine);
    var target = box.min.clone().add(box.max.clone()).multiplyScalar(0.5);
    let dir = new THREE.Vector3(1, 1, 1);
    var tergetCamera = target.clone().add(dir.multiplyScalar(1 * box.max.distanceTo(box.min)));
    _ViewCube.animateCamera(_ViewCube.scene.camera.position, tergetCamera, _ViewCube.scene.controls.target.clone(), target);
    //相机运动
  };
  //相机平滑跳转
