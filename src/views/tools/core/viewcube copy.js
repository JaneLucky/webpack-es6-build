...
export function ViewCube(scene, domid) {
  require('@/views/tools/style/'+SetDeviceStyle()+'/mainstyle.scss')
	...
	_ViewCube.GetCameraPose = function() {
		...
		var rotation = _ViewCube.scene.camera.rotation; //获取相机旋转
		var scale = _ViewCube.scene.camera.scale; //获取相机缩放
		var up = _ViewCube.scene.camera.up; //获取相机缩放
		var matrix = _ViewCube.scene.camera.matrix.elements; //获取相机缩放
		var matrixw = _ViewCube.scene.camera.matrixWorld.elements; //获取相机缩放
		var matrixwi = _ViewCube.scene.camera.matrixWorldInverse.elements; //获取相机缩放
		var matrixwi = _ViewCube.scene.camera.matrixWorldInverse.elements; //获取相机缩放
		var projectionMatrix = _ViewCube.scene.camera.projectionMatrix.elements; //获取相机缩放
		var projectionMatrixInverse = _ViewCube.scene.camera.projectionMatrixInverse.elements; //获取相机缩放
		var origin = _ViewCube.scene.controls.origin; //获取目标点 
		var originPosition = _ViewCube.scene.controls.originPosition; //获取目标点 
		var position0 = _ViewCube.scene.controls.position0; //获取目标点 
	...
	_ViewCube.animateCamera = function(current1, target1, current2, target2, params) {
		...
		if(params){
			// _ViewCube.scene.controls.auto = true;
			// _ViewCube.scene.controls.origin.x = origin.x
			// _ViewCube.scene.controls.origin.y = origin.y
			// _ViewCube.scene.controls.origin.z = origin.z
			// _ViewCube.scene.controls.originPosition.x = originPosition.x
			// _ViewCube.scene.controls.originPosition.y = originPosition.y
			// _ViewCube.scene.controls.position0.x = position0.x
			// _ViewCube.scene.controls.position0.y = position0.y
			// _ViewCube.scene.controls.position0.z = position0.z

			_ViewCube.scene.controls.target.x = params.target.x;
			_ViewCube.scene.controls.target.y = params.target.y;
			_ViewCube.scene.controls.target.z = params.target.z;
			
			_ViewCube.scene.camera.position.x = params.position.x
			_ViewCube.scene.camera.position.y = params.position.y
			_ViewCube.scene.camera.position.z = params.position.z

			_ViewCube.scene.camera.rotation.x = params.rotation.x
			_ViewCube.scene.camera.rotation.y = params.rotation.y
			_ViewCube.scene.camera.rotation.z = params.rotation.z
			_ViewCube.scene.camera.scale.x = params.scale.x
			_ViewCube.scene.camera.scale.y = params.scale.y
			_ViewCube.scene.camera.scale.z = params.scale.z
			_ViewCube.scene.camera.quaternion._x = params.quaternion._x
			_ViewCube.scene.camera.quaternion._y = params.quaternion._y
			_ViewCube.scene.camera.quaternion._z = params.quaternion._z
			_ViewCube.scene.camera.quaternion._w = params.quaternion._w 
			// _ViewCube.scene.camera.lookAt(target2)
			_ViewCube.scene.camera.up.x = params.up.x
			_ViewCube.scene.camera.up.y = params.up.y
			_ViewCube.scene.camera.up.z = params.up.z
			
			let mat = new THREE.Matrix4();
			mat.elements = params.matrix;
			console.log(mat)
			_ViewCube.scene.camera.matrix =mat;

			let matw = new THREE.Matrix4();
			matw.elements = params.matrixw;
			_ViewCube.scene.camera.matrixWorld =matw;
			
			let matwi = new THREE.Matrix4();
			matwi.elements = params.matrixwi;
			_ViewCube.scene.camera.matrixWorldInverse =matwi;

			let pmat = new THREE.Matrix4();
			pmat.elements = params.projectionMatrix;
			_ViewCube.scene.camera.matrixWorldInverse =pmat;
			
			let pmati = new THREE.Matrix4();
			pmati.elements = params.projectionMatrixInverse;
			_ViewCube.scene.camera.matrixWorldInverse =pmati;
			// _ViewCube.scene.controls.update();
			// _ViewCube.renderScene();
		}
