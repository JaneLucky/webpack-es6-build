...
export function SetDeleteListener(_Engine, list, status){
  for (let item of list) {
    ...
    keyName && status ? (_Engine.Measures && _Engine.Measures[keyName].OpenDrawDeleteListener()): (_Engine.Measures && _Engine.Measures[keyName].CloseDrawDeleteListener())
  }
}
...
export function SetMeshsClipPlane(_Engine, planes) {
  if(planes && planes.length){
    ...
    // 如果剖切功能开启
    if(_Engine && _Engine.Clipping && _Engine.Clipping.isActive){
