...
export function CaptureMark(bimengine) {
	...
	_captureMark.Active = function() {
		if (!_captureMark.isActive) {
			...
			switch (DeviceType) {
				case "Mobile":
					MobileController = CreateMobileController(CaptureMarkDom)
					MobileController.addEventListener('pointerdown', onMouseDownMobile);
					break;
				case "PC":
	...
	_captureMark.DisActive = function() {
		if (_captureMark.isActive) {
			...
			switch (DeviceType) {
				case "Mobile":
					MobileController.removeEventListener('pointerdown', onMouseDownMobile);
					MobileController && MobileController.remove() //删除坐标点dom
					break;
				case "PC":
	...
	//鼠标按下 - 移动端指示器
	function onMouseDownMobile(event) {
		event.preventDefault(); // 阻止默认的点击事件执行

		var left = MobileController.offsetLeft;
		var top = MobileController.offsetTop;
		//计算出鼠标的位置与元素位置的差值。
		var cleft = event.clientX - left;
		var ctop = event.clientY - top;
		document.addEventListener('pointermove',handelPosition)
		document.addEventListener('pointerup',()=>{
			document.removeEventListener("pointermove",handelPosition)
		})
		function handelPosition(evt){
			//计算出移动后的坐标。
			var moveLeft = evt.clientX - cleft;
			var moveTop = evt.clientY - ctop;
			//当移动位置在范围内时，元素跟随鼠标移动。
			MobileController.style.left = moveLeft + "px";
			MobileController.style.top = moveTop + "px";
			_captureMark.MouseEvent = {
				x: moveLeft,
				y: moveTop,
				event: {
					target: bimengine.scene.renderer.domElement
				}
			}
		}
	}
	...
	function Animate_MeasurePointPink(mouseEvent) {
		...
		if(DeviceType === "PC"){
			rayCaster  = new THREE.Raycaster(new THREE.Vector3(), new THREE.Vector3(), 0, 30);
		}
	...
	function getPinkType(scene, intersect, list, normal) {
		...
		if (line && line.distance <= cameraToLineDistance * 0.025) { //确定是到线
			PINK_DETAILS = {
				type: "line",
				val: GeometricOperation().PointProjectLine(intersect.point, new THREE.Vector3(line.start.x, line
						.start.y, line.start.z),
	...
	function CreateCaptureMarkDom(domName) {
		...
		polygon.setAttribute('style',
			'fill:rgba(0,255,0,0.2);stroke:rgba(0,255,0,1);stroke-width:2;pointer-events:none;')
		...
		AreaPolygon.setAttribute('style',
			'fill:rgba(0,255,0,0.2);stroke:rgba(0,255,0,1);stroke-width:2;pointer-events:none;')
	...
	function CreateMobileController(domName){
		var root = getRootDom(domName);
		let controller = document.createElement("div");
		controller.id = "MobileController";
		let width=50, height=50;
		controller.style.width = width + 'px';
		controller.style.height = height + 'px';
		controller.style.top = _container.clientHeight/2 - width/2 + 'px'
		controller.style.left = _container.clientWidth/2 - height/2 + 'px'

		// let arrow = 
		let NS_SVG = 'http://www.w3.org/2000/svg'
		let arrow_SVG = document.createElementNS(NS_SVG, 'svg')
		arrow_SVG.setAttribute('style', 'position:absolute;width:20px;height:20px;pointer-events:none;')
		let arrow = document.createElementNS(NS_SVG, "polygon")
		arrow.setAttribute('points', '0,0 10,0 0,10')
		arrow.setAttribute('style',
			'fill:#333333;pointer-events:none;')
		arrow_SVG.appendChild(arrow);
		controller.appendChild(arrow_SVG);
		let handle = document.createElement("div");
		handle.className = "ControllerHandle"
