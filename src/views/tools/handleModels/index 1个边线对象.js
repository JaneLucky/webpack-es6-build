...
export function HandleRequestModelSelect_(list, visible) {
	...
	let ModelEdges = handleEdgeModels()
	let ModelEdgesChild, newPositions;
	if(ModelEdges){
		ModelEdgesChild = ModelEdges.ElementInfos.children
		newPositions = visible?[]:[...ModelEdges.ElementInfos.positions]
	}
	...
	for (let i = 0; i < list.length; i++) {
		...
		if (model.instanceMatrix == null && model.geometry != null && model.geometry.groups[list[i][1]] != null) {
			...
			if(visible==true){
				model.visible = true;
				if(ModelEdges){
					Array.prototype.splice.apply(newPositions, [newPositions.length, model.ElementInfos[list[i][1]].EdgeList.length].concat(model.ElementInfos[list[i][1]].EdgeList));
				}
			} else{
				if(ModelEdges){
					let edgeItem = ModelEdgesChild.filter(item=>item.index === list[i][0])
					let info = edgeItem?edgeItem[0].ElementInfos[list[i][1]]:null
					let ins = info.EdgeList.map(item=>{return 0})
					Array.prototype.splice.apply(newPositions, [info.startIndex, info.EdgeList.length].concat(ins));
				}
			}
	...
	if(ModelEdges){
		ModelEdges.geometry.setAttribute(
			'position',
