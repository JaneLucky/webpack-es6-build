...
export function Render(bimengine) {
	...
	_render.DisplayEdge = function(enable) {
		...
		if (enable == true) {
			...
			for (var rootmodel of rootmodels) {
				...
				for (var model of rootmodel.ElementInfos) {
					let lineModel = {
						...
						endIndex : positions.length + model.EdgeList.length -1,
						EdgeList : model.EdgeList,
					}
					group.ElementInfos.push(lineModel)
					// model.EdgeList.forEach(o => {
					// 	positions.push(o);
					// })
					Array.prototype.splice.apply(positions, [positions.length, model.EdgeList.length].concat(model.EdgeList));
				}
				ElementInfos.children.push(group)
			}
			...
			//--------------核心--------------
			ElementInfos.positions = positions
			geometry.setAttribute(
				'position',
	...
	function CreatorSetDialog() {
		...
		input_item_1.onchange = (e) => {
			...
			bimengine.RenderSAO.saoPass.params.output = 0
			bimengine.RenderSAO.saoPass.params.saoBias= 1
			bimengine.RenderSAO.saoPass.params.saoBlur= 0
