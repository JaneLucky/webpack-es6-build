...
export function Render(bimengine) {
	...
	_render.DisplayEdge = function(enable) {
		...
		// var rootmodels = bimengine.scene.children.filter(x => x.name == "ModelEdges");
		// rootmodels.map(item=>{
		// 	item.visible = enable
		// })
		...
		if(enable){
			...
			for (const Edge of EdgeList) {
				if(!Edge.Created){
					Edge.Created = true
					createLineSegments(Edge.Indexs, Edge.EdgeList, null, material, clippingPlanes)
	...
	//创建边线模型
	function createLineSegments(index, positions, ElementInfos, material, clippingPlanes) {
		let geometry = new THREE.BufferGeometry()
		geometry.setAttribute(
			'position',
