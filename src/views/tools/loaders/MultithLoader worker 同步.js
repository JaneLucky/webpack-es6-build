...
export function MultithLoadGlbJsonList(Scene, rootPath, relativePath, type, option) {
    ...
    switch (postMsg.flag) {
      case 'modelLoad':
        ...
        if(postMsg.meshList && postMsg.meshList.length){
          ...
          mergeBufferModel(Scene, allMeshs, postMsg.fullPath, postMsg.index);
        }else{
          worker.postMessage({
            txt: 'continueLoad',
            index: postMsg.index+1
          });
        }
        break;
      case 'oneModelLoad':
  ...
  worker.onerror = function (event) {	 
	...
	//合并模型 
	function mergeBufferModel(Scene, meshs, path, index) {
