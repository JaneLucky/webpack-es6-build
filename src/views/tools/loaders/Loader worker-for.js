			...
			for(let i=0; i<rootmodels.length;i++){
				if(rootmodels[i].TypeName == "Mesh"){
					for(let j=0;j<rootmodels[i].ElementInfos.length;j++){
						...
						worker.postMessage({
						geometry : new THREE.EdgesGeometry(rootmodels[i].meshs[j].geometry, 89).attributes.position.array,
						matrix: rootmodels[i].meshs[j].matrix.clone().elements
						});//将复杂计算交给子线程,可以理解为给参数让子线程去操作。
						worker.onmessage = function(e){
							rootmodels[i].ElementInfos[j].EdgeList = e.data
							worker.terminate()
						}
					...
					for(let j=0;j<rootmodels[i].ElementInfos.length;j++){
						...
						// var worker = new Worker('./file.worker.js');
						worker.postMessage({
						geometry : new THREE.EdgesGeometry(rootmodels[i].meshs.geometry, 89).attributes.position.array,
						matrix: rootmodels[i].ElementInfos[j].matrix.clone().elements
						});//将复杂计算交给子线程,可以理解为给参数让子线程去操作。
						worker.onmessage = function(e){
							rootmodels[i].ElementInfos[j].EdgeList = e.data
							worker.terminate()
